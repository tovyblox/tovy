...
export async function handler(
	...
	const userid = await getRobloxUserId(req.body.username).catch(() => null);
	if (!userid || Array.isArray(userid)) return res.status(400).json({ success: false, error: 'Invalid username' });
