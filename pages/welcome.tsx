...
const Login: NextPage = ({}) => {
  ...
  return (
    <div className="flex bg-infobg h-screen bg-no-repeat bg-cover bg-center">
      ...
      <div className={`bg-white h-auto w-11/12 sm:w-4/6 md:3/6 xl:w-5/12 block mx-auto my-auto rounded-3xl `} >
        <div className="m-6">
          ...
          <div className="mt-7">
            ...
            <div className="grid grid-cols-5 md:grid-cols-7 lg:grid-cols-11 xl:grid-cols-10 gap-y-3 mb-8 mt-2">
				{colors.map((color, i) => (
					<a key={i} onClick={() => setSelectedColor(color)} className={`h-12 w-12 block rounded-full transform ease-in-out ${color} ${selectedColor === color ? 'border-black border-4' : ''}`} />
				))}
			</div>
          </div>
          <div className="flex">
            ...
            <button className="ml-auto bg-[#2196F3] py-3 text-sm rounded-xl px-6  text-white font-bold hover:bg-blue-300 transition hover:scale-90">
              Continue
            </button>
          </div>
        </div>
